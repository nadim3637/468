...
import { ArrowLeft, Clock, AlertTriangle, ExternalLink, CheckCircle, XCircle, Trophy, BookOpen, Play, Lock, ChevronRight, ChevronLeft, Save, X, Maximize, Volume2, Square, Zap, StopCircle, Globe, RefreshCw } from 'lucide-react';
...
export const LessonView: React.FC<Props> = ({ 
  ...
  const [localContent, setLocalContent] = useState<LessonContent | null>(content);
  ...
  const handleLanguageToggle = async () => {
      // If currently Hindi, switch to English
      if (language === 'Hindi') {
          setLanguage('English');
          return;
      }
      ...
      // Switching to Hindi
      const hasHindi = localContent?.schoolPremiumNotesHtml_HI || localContent?.competitionPremiumNotesHtml_HI || localContent?.manualMcqData_HI || (localContent?.type === 'MCQ_SIMPLE' && localContent.manualMcqData_HI);
      ...
      try {
          const sourceText = localContent?.content || localContent?.schoolPremiumNotesHtml || "";
          
          if (!sourceText) {
             // If no text content, maybe it's purely PDF/Video, ignore or simple toggle
             setLanguage('Hindi');
             return;
          }

          const prompt = `
          Generate PREMIUM HINDI NOTES for Class ${classLevel} Subject ${subject.name}, Chapter: "${chapter.title}".
          Target: 1500 Words.
          
          STYLE: "Gemini Style" - Conversational, Deep Analysis, Detailed.
          Language: Hindi (Devanagari) mixed with English terms (Hinglish) for clarity.
          
          STRUCTURE:
          1. 🌟 Introduction
          2. 📘 Detailed Concept Explanation (Step-by-step)
          3. 🧠 Deep Dive Analysis
          4. 🧪 Examples
          5. ⚠️ Exam Alerts
          6. 📝 Summary
          
          Format as valid Markdown/HTML. Use bolding and lists.
          `;

          const hindiText = await callGroqApi([
               { role: "system", content: "You are an expert Indian teacher." },
               { role: "user", content: prompt }
          ]);

          const updatedContent = { ...localContent! };
          
          if (updatedContent.type === 'NOTES_PREMIUM' || updatedContent.type === 'NOTES_SIMPLE') {
               updatedContent.schoolPremiumNotesHtml_HI = hindiText;
               // Fallback if competition
               if (user?.syllabusMode === 'COMPETITION') {
                   updatedContent.competitionPremiumNotesHtml_HI = hindiText;
               }
          } else {
               // Generic fallback
               updatedContent.schoolPremiumNotesHtml_HI = hindiText;
          ...
          if (user) {
               ...
               const key = `nst_content_${user.board}_${classLevel}${streamKey}_${subject.name}_${chapter.id}`;
               await saveChapterData(key, updatedContent);
               alert("✅ Hindi Notes Generated & Saved Permanently!");
          }

      } catch (e) {
          console.error("Hindi Gen Error", e);
          alert("Failed to generate Hindi notes. Please check internet.");
      } finally {
                  ...
                  <header className="bg-white/95 backdrop-blur-md text-slate-800 p-4 absolute top-0 left-0 right-0 z-10 flex items-center justify-between border-b border-slate-100 shadow-sm">
                      ...
                      <div className="flex items-center gap-1">
                          ...
                          >
                              ...
                              {isTranslating ? 'Translating...' : (language === 'English' ? 'Hindi (हिंदी)' : 'English')}
                          </button>
