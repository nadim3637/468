...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      // Sync to storage is handled in toggler or effect if loaded
      if (!isInitializing) storage.setItem('nst_dark_mode', darkMode.toString());
  }, [darkMode, isInitializing]);
  ...
  useEffect(() => {
    if (!isInitializing) storage.setItem('nst_active_student_tab', studentTab);
  }, [studentTab, isInitializing]);
  ...
  useEffect(() => {
      storage.getItem<string>('nst_app_version').then(storedVersion => {
          if (!storedVersion || storedVersion !== APP_VERSION) {
              console.log(`Updating Local Version: ${storedVersion} -> ${APP_VERSION}`);
              storage.setItem('nst_app_version', APP_VERSION);
          }
      });
  }, []);
  ...
  useEffect(() => {
      ...
      if (deletedKeys.length > 0) {
          ...
          if (newDeletedKeys.length !== deletedKeys.length) {
              ...
              storage.setItem('nst_system_settings', updatedSettings);
          }
      }
  ...
  useEffect(() => {
      if (isOnline) {
          storage.getItem<string>('nst_pending_sync_results').then(pendingStr => {
              if (pendingStr) {
                  try {
                      ...
                      if (Array.isArray(pending) && pending.length > 0) {
                          ...
                          storage.removeItem('nst_pending_sync_results');
      ...
      const unsubscribe = subscribeToSettings((newSettings) => {
          if (newSettings) {
              ...
              if (newSettings.forceRefreshTimestamp) {
                  storage.getItem('nst_last_refresh_ts').then(lastRefresh => {
                      if (lastRefresh !== newSettings.forceRefreshTimestamp) {
                          storage.setItem('nst_last_refresh_ts', newSettings.forceRefreshTimestamp);
              ...
              const checkVersion = async () => {
                  const currentVersion = (await storage.getItem('nst_app_version')) || APP_VERSION;
                      ...
                      if (newSettings.latestVersion > currentVersion) {
                          ...
                          if (isUpdateAvailable) {
                              ...
                              if (!referenceTime) {
                                  ...
                                  const firstSeen = await storage.getItem(key);
                                  if (!firstSeen) {
                                      referenceTime = now;
                                      await storage.setItem(key, now.toString());
                                  } else {
                                      referenceTime = parseInt(firstSeen as string);
                                  }
                              }
                              ...
                              if (now >= deadline) {
                                  ...
                                  const lastDismissedStr = await storage.getItem(`nst_update_dismissed_${newSettings.latestVersion}`);
                                  if (!lastDismissedStr) {
                                      shouldShow = true; // First time seeing it (since dismissal reset)
                                  } else {
                                      const lastDismissed = parseInt(lastDismissedStr as string);
                                      let freqDuration = 0;
          ...
          getUserData(state.user.id).then(cloudUser => {
             if (cloudUser) {
                 ...
                 if (currentStr !== cloudStr) {
                     ...
                     storage.setItem('nst_current_user', cloudUser);
  ...
  useEffect(() => {
      const loadInitialData = async () => {
          ...
          try {
              ...
              const storedSettings = await storage.getItem<SystemSettings>('nst_system_settings');
              if (storedSettings) {
                  loadedSettings = { ...state.settings, ...storedSettings };
              }

              // 2. Load Dark Mode
              const storedDarkMode = await storage.getItem<string>('nst_dark_mode');
              if (storedDarkMode === 'true') setDarkMode(true);

              // 3. Load User
              const storedUser = await storage.getItem<User>('nst_current_user');
              const hasAcceptedTerms = await storage.getItem('nst_terms_accepted');
              ...
              if (storedUser) {
                  ...
                  const lastTracker = await storage.getItem<string>('nst_last_daily_tracker_date');
                  if (lastTracker !== today) {
                      queue.push('TRACKER');
                  }

                  // 3. Daily Challenge (Once per day)
                  const lastChallenge = await storage.getItem<string>('nst_last_daily_challenge_date');
                  if (lastChallenge !== today) {
                      queue.push('CHALLENGE');
                  }
              }

              const hasSeenWelcome = await storage.getItem('nst_has_seen_welcome');
              setPopupQueue(queue);
              ...
              if (storedUser) {
                  ...
                  if (user.role !== 'ADMIN' && user.isPremium && user.subscriptionEndDate && new Date(user.subscriptionEndDate) <= new Date()) {
                        ...
                        await storage.setItem('nst_current_user', user);
                        saveUserToLive(user);
                  }

                  if (!user.progress) user.progress = {};
                  if (user.isLocked) { 
                        await storage.removeItem('nst_current_user'); 
                        setAlertConfig({isOpen: true, message: "Account Locked. Please contact Admin."}); 
                        setState(prev => ({...prev, settings: loadedSettings }));
                  } else {
  ...
  useEffect(() => {
    ...
    const loadTimer = async () => {
        const today = new Date().toDateString();
        const storedDate = await storage.getItem<string>('nst_timer_date');
        const storedSeconds = await storage.getItem<string>('nst_daily_study_seconds');
        const secondsVal = parseInt(storedSeconds || '0');

        if (storedDate !== today) {
            storage.setItem('nst_timer_date', today);
            storage.setItem('nst_daily_study_seconds', '0');
            setDailyStudySeconds(0);
        } else {
            setDailyStudySeconds(secondsVal);
        }
    };
    loadTimer();
    ...
    if (state.user) {
        interval = setInterval(() => {
            setDailyStudySeconds(prev => {
                const next = prev + 1;
                storage.setItem('nst_daily_study_seconds', next.toString());
  ...
  useEffect(() => {
      ...
      const applyTheme = async () => {
          if (state.user) {
              ...
              try {
                  const lbData = await storage.getItem<any[]>('nst_leaderboard');
                  if (lbData) {
  ...
  // --- LOGGING SYSTEM ---
  const logActivity = async (action: string, details: string, overrideUser?: User) => {
      ...
      const storedLogs = await storage.getItem<ActivityLogEntry[]>('nst_activity_log');
      const logs = storedLogs || [];
      // Keep last 500 logs
      const updatedLogs = [...logs, newLog].slice(-500); 
      storage.setItem('nst_activity_log', updatedLogs);
  };
  ...
  const handleAcceptTerms = () => {
      storage.setItem('nst_terms_accepted', 'true');
      setShowTerms(false);
      storage.getItem('nst_has_seen_welcome').then(hasSeenWelcome => {
          if (!hasSeenWelcome) setState(prev => ({ ...prev, showWelcome: true }));
      });
  };

  const handleStartApp = () => {
    storage.setItem('nst_has_seen_welcome', 'true');
  ...
  const handleLogin = (user: User) => {
    ...
    if (!state.originalAdmin) {
        storage.setItem('nst_current_user', user);
    }
  ...
  const handleLogout = () => {
    ...
    storage.removeItem('nst_current_user');
  ...
  const handleContentGeneration = async (type: ContentType, count?: number, forcePay: boolean = false) => {
    ...
    if (type === 'NOTES_HTML_FREE' || type === 'NOTES_HTML_PREMIUM' || type === 'NOTES_IMAGE_AI') {
        ...
        if (!contentData) {
            const stored = await storage.getItem(mainKey);
            if (stored) contentData = stored;
        }
            ...
            if (!state.originalAdmin) {
                ...
                if (storedUsers) {
                    ...
                    if (idx !== -1) { 
                        ...
                        storage.setItem('nst_users', allUsers); 
                    }
                }
                // Sync to Live
                saveUserToLive(updatedUser);
  ...
  const handleWeeklyTestComplete = async (score: number, total: number, answers: Record<number, number>) => {
    ...
    // Save Attempt
    const startTimeStr = await storage.getItem<string>(`weekly_test_start_${activeWeeklyTest.id}`);
    ...
    const storedAttempts = await storage.getItem<string>(key);
    ...
    // Cleanup Local Timer
    storage.removeItem(`weekly_test_start_${activeWeeklyTest.id}`);
  };
  ...
  const handlePopupClose = (type: string) => {
      ...
      if (type === 'TRACKER') {
          storage.setItem('nst_last_daily_tracker_date', new Date().toDateString());
      } else if (type === 'CHALLENGE') {
          storage.setItem('nst_last_daily_challenge_date', new Date().toDateString());
      ...
      return (
          <div className="min-h-screen flex items-center justify-center bg-white">
              <div className="flex flex-col items-center">
                  ...
                  <p className="font-bold text-slate-600">Initializing App...</p>
              </div>
          </div>
      );
  }
      ...
      {showUpdatePopup && state.settings.latestVersion && state.settings.updateUrl && (
          <UpdatePopup 
              ...
              onClose={() => {
                  ...
                  storage.setItem(`nst_update_dismissed_${state.settings.latestVersion}`, Date.now().toString());
              }} 
          />
      )}
    </div>
